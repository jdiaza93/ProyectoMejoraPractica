...
<body>
    ...
    <main>
        <div class="Histori">
            <a href="PagIni.php">Inicio</a> > <a href="ConfirHoraLab.php">Consultar horas agendadas</a> 
        </div>
        ...
        $sql=mysqli_query($conn, "SELECT * FROM horas_holter WHERE ID_Usuario='$UsuID' AND Estado='Sin confirmar' OR Estado='Confirmada' ORDER BY Fecha ASC");
        if ($sql->num_rows>0){
            while($row=mysqli_fetch_array($sql)){
                ...
                echo "<div id='CuadHoras'>
                        <div class='Conten'>
                            ...
                            <div>
                                <p><strong>Dia</strong>: $Fecha <strong>Hora</strong>: $Hora</p><br><br>
                            </div>
                            <div>
                                <p><strong>Estado</strong>: {$row["Estado"]}</p> 
                            </div>
                            <div>".($row["Estado"] == "Sin confirmar" ? 
                                "<form action='ConfirmaHoras.php' method='POST' name='ConfirmHoraHol'>
                                    <input type='hidden' id='IdHora' name='IdHora' value='{$row["ID"]}'>
                                    <button type='submit' class='btn btn-primary btn-lg' name='ConfirmHoraHol'>Confirmar hora</button>
                                </form>" : 
                                "")
            ...
            echo "<br><div id='MensSinHoras'>No tiene horas agendadas. Si quiere  agendar, seleccione 'Agendar Exámenes de Laboratorio' en el menú de Exámenes de laboratorio.</div>";
        }
        ?>        
