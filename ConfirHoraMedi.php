...
<body>
    ...
    <main>
        <div class="Histori">
            <a href="PagIni.php">Inicio</a> > <a href="ConfirHoraMedi.php">Consultar horas agendadas</a> 
        </div>
        <h1>Confirmación de horas</h1>
        <div class='MensRecord'><span style='color:red;'>Importante:</span> Acuérdese de llevar un documento de identificación el día de su hora como cédula de identidad, pasaporte o certificado de nacimiento.</div><br>
        <?php
        $UsuID=$_SESSION['ID_Usu'];
        $sql=mysqli_query($conn, "SELECT horas_medicos.ID, Nombres, NomEspec, Fecha, Hora, Estado FROM horas_medicos LEFT JOIN datosdoctores ON (horas_medicos.ID_Doctor=datosdoctores.ID) LEFT JOIN especialidades ON (horas_medicos.ID_Especialidad=especialidades.ID) WHERE ID_Usuario=$UsuID AND Estado='Sin confirmar' OR Estado='Confirmada' ORDER BY Fecha ASC");
        if ($sql->num_rows>0){
                ...
                echo "<div id='CuadHoras'>
                        <div class='Conten'>
                            ...
                            <div>".($row["Estado"] == "Sin confirmar" ? 
                                "<form action='ConfirmaHoras.php' method='POST' name='ConfirmHoraMed'>
                                    <input type='hidden' id='IdHora' name='IdHora' value='{$row["ID"]}'>
                                    <button type='submit' class='btn btn-primary btn-lg' name='ConfirmHoraMed'>Confirmar hora</button>
                                </form>" : 
                                "")
            ...
            echo "<br><div id='MensSinHoras'>No tiene horas agendadas. Si quiere  agendar, seleccione 'Agendar Horas Médicas' en el menú de Horas médicas.</div>";
        }
        ?>      
